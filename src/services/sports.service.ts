...
class SportsService {
  ...
  getMatches(sportId: string, competitionId: string) {
    return sportsApi.get(
      `/get-matches?EventTypeID=${sportId}&CompetitionID=${competitionId}`
    );
  }
  ...
  getMatchList(sportId = '', status = 'all', limit = '') {
    return api.get(
      `/get-match-list?sportId=${sportId}&status=${status}&limit=${limit}`
    );
  }
  ...
  getSeriesWithMatch(EventTypeID: string | number) {
    return api.get(
      `get-series-with-market-with-date?EventTypeID=${EventTypeID}`
    );
  }
  ...
  fancyData(matchId: number) {
    return sportsApi.get(
      `${
        import.meta.env.VITE_APP_API_SPORTS_FANCY_URL
      }fancy-data?MatchID=${matchId}`
    );
  }
}
