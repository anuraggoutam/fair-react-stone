...
const Worli = (props: any) => {
  ...
  const limitList: any = {
    ...
    '56 Charts': { min: 1, max: 1 },
    '64 Charts': { min: 1, max: 1 },
    'COMMON SP': { min: 1, max: 1 },
    'COMMON DP': { min: 1, max: 1 },
    'COLOR DP': { min: 1, max: 1 },
    Trio: { min: 1, max: 1 },
    Cycle: { min: 1, max: 2 },
  };
  ...
  const onBet = (isBack = false, item: any) => {
    ...
    if (userState.user.role === RoleType.user) {
      ...
      const activeMarketCondition = limitList?.[activeTab] || {
        min: 1,
        max: 1,
      };
  ...
  const numberLayout = (marketNameSerach: string, marketRemarkSearch = '') => {
    ...
    const marketList: any =
      marketRemarkSearch == ''
        ? liveMatchData?.defaultMarkets?.filter((Item: any) =>
            Item.MarketName.includes(marketNameSerach)
          )
        : liveMatchData?.defaultMarkets?.filter((Item: any) =>
            Item.remark.includes(`Welcome ${marketNameSerach}`)
          );
      ...
      <table className="table coupon-table table table-bordered">
        <thead style={{ borderBottom: '0px' }}>
          <tr>
            ...
            >
              <div
                style={{ width: '97%', fontSize: '18px', textAlign: 'center' }}
              >
        ...
        <tbody data-title="SUSPENDED" className={`card32btbody ${clsstatus}`}>
          <tr style={{ height: '89px' }} key={4} className={`${heightdata}`}>
              ...
              return (
                <td
                  ...
                  className={`text-center bet-action back ${
                    selectionId.indexOf(parseInt(Item.SelectionId)) > -1
                      ? 'bg-green'
                      : ''
                  }`}
                  onClick={() => onBet(true, ItemMarket)}
                >
                  <span className="d-block card-number">
                    {Item.RunnerName.replace(' Single', '')
                      .replace(' Pana', '')
                      .replace(' SP', '')
                      .replace(' DP', '')
                      .replace(' Cycle', '')
            ...
            {['SP', 'DP', '56 Charts', '64 Charts', 'COLOR DP'].indexOf(
              marketNameSerach
            ) > -1 &&
              marketList?.map((ItemM: any, key: number) => {
                ...
                return (
                  ...
                  >
                    <span className="d-block card-number">
                      {Item.RunnerName.replace('Charts ', '')}
                    </span>
                  </td>
                );
              })}
  ...
  const numberLayout2 = (marketNameSerach: string) => {
    const marketList: any = liveMatchData?.defaultMarkets?.filter((Item: any) =>
      Item.MarketName.includes(marketNameSerach)
    );
    const ItemMarket: any = lastOdds?.[1] || {};
    const clsstatus =
            ...
            {marketList?.map((ItemM: any, key: number) => {
              ...
              return (
                <td
                  ...
                  onClick={() => onBet(true, ItemMarket)}
                >
                  <span className="d-block" style={{ fontSize: '18px' }}>
                    {Item.RunnerName}
                  </span>
                  {key == 10 && (
            ...
            {marketList?.map((ItemN: any, key: number) => {
              ...
              return (
                <td
                  ...
                  onClick={() => onBet(true, ItemMarket)}
                >
                  <button className="btn btn-primary text-uppercase custom-trio">
                    All Trio
                  </button>
                  <div>
                    <div className="ubook text-danger">
  ...
  const numberLayoutABR = (
    marketNameSerach: string,
    marketRemarkSearch = ''
  ) => {
    const heightdata = '';
    const marketList: any =
    ...
    <div className="container ">
      <div className="row casino-32A ">
        ...
        {activeTab == 'Single' && (
          <>
            <div
              className="col-lg-8 m-b-10 main-market  bg-gray"
              style={{ padding: '0px' }}
            >
              <div className="live-poker">{numberLayout('Single')}</div>
            </div>
            <div
              className="col-lg-4 m-b-10 main-market  bg-gray"
              style={{ padding: '0px' }}
            >
