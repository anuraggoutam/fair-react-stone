...
const Casinowar = (props: any) => {
  ...
  const cardImageUrl =
    'https://dzm0kbaskt4pv.cloudfront.net/v11/static/front/img/cards/';
  ...
  const laybacklayout = (from: number, to: number) => {
    ...
    return [
      ...
      'Heart',
      'Diamond',
    ].map((ItemNew: any, key: number) => {
      let title =
        ItemNew == 'Black' ? (
          <span>
            {ItemNew} {isMobile ? activeTab : ''}{' '}
            <img src={`imgs/casino/club.png`} style={{ width: '25px' }} />
            <img src={`imgs/casino/spade.png`} style={{ width: '25px' }} />
          </span>
        ) : (
          `${ItemNew} ${isMobile ? activeTab : ''} `
        );
      title =
        ItemNew == 'Red' ? (
          <span>
            {ItemNew} {isMobile ? activeTab : ''}{' '}
            <img src={`imgs/casino/diamond.png`} style={{ width: '25px' }} />
            <img src={`imgs/casino/heart.png`} style={{ width: '25px' }} />
          </span>
        ) : (
          title
        );
          ...
          >
            <div className="d-flex" style={{ justifyContent: 'space-between' }}>
              {key > 4 ? (
                <b>
                  <img
                    src={`imgs/casino/${ItemNew.toLowerCase()}.png`}
                    style={{ width: '25px' }}
                  />
                </b>
              ) : (
            ...
            (ItemFake: any, keyFake: number) => {
              ...
              const clsstatus =
                ...
                Item.gstatus === '0'
                  ? 'suspended'
                  : '';
              return !isMobile || ItemFake == activeTab ? (
                <td
                  className={`back teen-section ${clsnamename}`}
                  key={keyFake}
                >
                  <button
                    className={`back ${clsstatus}`}
  ...
  return (
    <div className="container " style={{ marginTop: '-10px' }}>
      <div className="row casino-32A ">
        {lastOdds && isMobile ? (
          <div
            className="col-lg-12 col-12 main-market"
            style={{ padding: '0px' }}
          >
            <div className="card-inner card-dtl-mobile">
              {' '}
        ...
        {isMobile ? (
          <div
            className="col-lg-12 col-12 m-b-10 main-market"
            style={{ padding: '0px' }}
          >
        ...
        <div
          ...
          style={{ padding: isMobile ? '3px' : '0px' }}
        >
          <div className="live-poker">
            <table
              className={`table coupon-table table table-bordered suspendwidth ${
                isMobile ? 'bg-gray' : ''
              }`}
            >
              {!isMobile ? (
                <thead>
                  <tr style={{ background: '#fff' }}>
                    ...
                    <th className={`${'box-1'}`} style={{ border: 'none' }}>
                      {' '}
                      <img
                        src={`${cardImageUrl}${
                          lastOdds?.['cards']?.C1 || 1
