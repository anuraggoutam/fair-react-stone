...
const TestTp = (props: any) => {
  ...
  const laybacklayout = () => {
    return [
      ...
      const filterMarketLiveData =
        lastOdds?.market?.filter(
          (ItemLive: any) => ItemLive.nation == `${ItemNew}`
        )?.[0] || {};
      const status =
        !filterMarketLiveData?.dstatus ||
        filterMarketLiveData?.dstatus == 'False'
          ? 'suspended'
          : '';
      return (
        <tr key={key} className={`sus60perce ${status}`}>
          <td className={'box-4'} style={{ padding: isMobile ? '3px' : '' }}>
            ...
            >
              ...
              {!isMobile ? (
                <Limitinfo
                  nameString={ItemNew}
                  min={liveMatchData?.min}
                  max={liveMatchData?.max}
                />
              ) : (
                ''
              )}
  ...
  const commonLayout = (market: any) => {
    ...
    const filterMarketLiveData =
      liveMatchData?.event_data?.market?.filter(
        (ItemLive: any) => ItemLive.nation == market
      )?.[0] || {};
      ...
      return (
        <td className={`back teen-section ${clsnamename}`} key={keyFake}>
          <button
            ...
            onClick={() => onBet(true, liveObj)}
            style={{ height: isMobile ? '43px' : '45px' }}
          >
            ...
            <table className="table coupon-table table table-bordered">
              <thead style={{ borderBottom: '0px' }}>
                <tr>
                  ...
                  <td
                    colSpan={3}
                    className={'back text-center fw600'}
