...
const Baccarat = (props: any) => {
  ...
  const onBet = (isBack = false, item: any) => {
    ...
    if (userState.user.role === RoleType.user) {
      ...
      if (oddsVal <= 0) return;
      if (!item.gstatus || item.gstatus == '0' || item.gstatus == 'SUSPENDED')
        return;
      dispatch(
        betPopup({
          isOpen: true,
  ...
  const layouttop = () => {
    return [5, 6, 7, 8, 9].map((Item: any, index: number) => {
      ...
      classgrid =
        liveMatchData.slug == 'baccarat' && isMobile ? 'col-25' : classgrid;
      classgrid =
        liveMatchData.slug == 'baccarat2' && isMobile ? 'col-20' : classgrid;
  ...
  const displayCard = (classn: any, cardIndex: any) => {
    ...
    return (
      <div className={`player-card ${classn}`} style={{ display: 'flex' }}>
        {cardIndex.map((Item: any, index: number) => {
          const clsbacrrat2 =
            Item == 5 ? 'rrotate' : Item == 6 ? 'rrotate' : '';
          return (
            <img
              key={index}
  ...
  const mainmarket = () => {
    ...
    return marketIndex.map((marketIndexItem: any, key: number) => {
      const Item =
        liveMatchData?.defaultMarkets?.[marketIndexItem]?.Runners?.[0] || {};
  ...
  const cornermarket = (marketIndex: any, key: number) => {
    const market =
      liveMatchData?.defaultMarkets?.[marketIndex]?.Runners?.[0] || {};
    const ItemMarket: any = lastOdds?.[market.SelectionId] || {};
    const clsdesign =
      market.RunnerName == 'Player Pair' ? 'player-pair' : 'banker-pair';
    const statusclassm =
      !ItemMarket.gstatus ||
      ItemMarket.gstatus == '0' ||
      ItemMarket.gstatus == 'SUSPENDED'
    ...
    <>
      {isMobile ? (
        <div className="row row6 p10 bg-theme mt--50">
          <div
            className="col-12 statistics-title"
            onClick={() => updatechartclass()}
          >
            Statistics{' '}
            {chartclass != '' ? (
      ...
      {lastOdds && (
        <div className="baccarat bg-gray mb-10 ">
          <div className="p15">
            <div className="row">
