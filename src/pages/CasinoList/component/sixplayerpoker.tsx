...
const Sixplayerpoker = (props: any) => {
  ...
  const onBet = (isBack = false, item: any) => {
    ...
    if (userState.user.role === RoleType.user) {
      ...
      if (oddsVal <= 0) return;
      if (!item.gstatus || item.gstatus == 'SUSPENDED' || item.gstatus == '0')
        return;
      dispatch(
        betPopup({
          isOpen: true,
  ...
  const laybacklayout = () => {
    ...
    return finalMarkets.map((ItemNew: any, key: number) => {
      ...
      const clsstatus =
        !Item.gstatus || Item.gstatus === '0' || Item.gstatus === 'CLOSED'
          ? 'suspended'
          : '';
      const card =
        cards[`C${key + 1}`] &&
      ...
      return (
        <div
          className="col-lg-6 col-6 mb-10 p0"
          style={{ paddingRight: key % 2 == 0 ? '10px' : '0px' }}
          key={key}
        >
          <button
            className={`bt-action btn-theme ${clsstatus}`}
            ...
            <span className="patern-name">
              ...
              {card && (
                <span className="card-icon m-l-20">
                  {replacecardstring(card)}{' '}
                  {card.includes('CC') && (
                    <span className="card-black">{'}'}</span>
                  )}{' '}
                  {card.includes('HH') && <span className="card-black">]</span>}{' '}
                  {card.includes('DD') && (
                    <span className="card-red">{'{'}</span>
                  )}{' '}
                  {card.includes('SS') && <span className="card-red">[</span>}
                </span>
              )}
              {card2 && (
                <span className="card-icon m-l-5">
                  {replacecardstring(card2)}{' '}
                  {card2.includes('CC') && (
                    <span className="card-black">{'}'}</span>
                  )}
                  {card2.includes('HH') && (
                  ...
                  {card2.includes('SS') && <span className="card-red">[</span>}
                </span>
              )}
            </span>
  ...
  const patterns = () => {
    ...
    return [1, 2, 3, 4, 5, 6, 7, 8, 9].map((ItemIndex: any) => {
      ...
      return runners.map((ItemNew: any, key: number) => {
        ...
        return (
          <div
            className="col-lg-4 col-4 mb-10 p0"
            style={{ paddingRight: key % 2 == 0 ? '10px' : '0px' }}
            key={key}
          >
            <button
              ...
              onClick={() => onBet(true, Item)}
            >
              <span className="patern-name">{ItemNew.RunnerName}</span>
              <span className="point">{Item.rate}</span>
              {isMobile ? (
                <p className="m-b-0 m-t-5 text-right min-max">
                  <span>
  ...
  return (
    <div
      className="container hands-pattern-container"
      style={{ marginTop: isMobile ? '-10px' : '' }}
    >
      <div className="row ">
        {lastOdds && lastOdds.desc != '' && (
          <div
            className="col-lg-12 main-market text-center tx-white bg-theme"
            style={{ padding: '10px' }}
          >
